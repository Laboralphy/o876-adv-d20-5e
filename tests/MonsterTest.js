...
describe('Ghast', function () {
    it('should apply paralyzed condition', function () {
        ...
        ghast.attack(goblin)
        expect(goblin.store.getters.getConditionSet.has(CONSTS.CONDITION_PARALYZED)).toBeTrue()
        ...
        expect(goblin.store.getters.getConditionSet.has(CONSTS.CONDITION_PARALYZED)).toBeTrue()
        expect(goblin.store.getters.getConditionSet.has(CONSTS.CONDITION_UNCONSCIOUS)).toBeFalse()
        expect(ghast.store.getters.isTargetAutoCritical).toBeTrue()
        expect(atk2.critical).toBeTrue()
        ghast.processEffects()
        goblin.processEffects()
        expect(goblin.store.getters.getConditionSet.has(CONSTS.CONDITION_PARALYZED)).toBeFalse()
    })
    it('should be poisonned when approachin ghast within 5 ft', function () {
        ...
        expect(gobVeryFar.store.getters.getConditionSet.has(CONSTS.CONDITION_POISONED)).toBeFalse()
        ...
        expect(gobJinxed.store.getters.getConditionSet.has(CONSTS.CONDITION_POISONED)).toBeTrue()
        expect(gobLucky.store.getters.getConditionSet.has(CONSTS.CONDITION_POISONED)).toBeFalse()
    })
})
...
describe('vampire', function () {
    it('should not be attacked by charmed targets', function () {
        ...
        expect(soldier.store.getters.getConditionSet.has(CONSTS.CONDITION_CHARMED)).toBeTrue()
        const a = soldier.attack()
        expect(a.failed).toBeTrue()
        expect(a.failure).toBe('ATTACK_OUTCOME_CONDITION')
        expect(soldier.store.getters.getConditionSet.has(CONSTS.CONDITION_CHARMED)).toBeTrue()
        vampire.attack()
        expect(soldier.store.getters.getConditionSet.has(CONSTS.CONDITION_CHARMED)).toBeFalse()
    })
})
